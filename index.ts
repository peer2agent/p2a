...
class WalletTracker {
  ...
  calculateAssetDistribution(outputData: any): void {
    ...
    distribution.forEach((d: any) =>
      console.log(
        `Asset ID: ${d.id}, Symbol: ${d.symbol}, Quantity: ${d.quantity.toFixed(
          6
        )}, Total Price: ${d.totalPrice.toFixed(2)} USDC, Distribution: ${
          d.percentage
        }%`
      )
    );

    this.storeDistributionInJsonFile(distribution);
  }
  ...
  storeDistributionInJsonFile(distribution: any): void {
    const outputDir = path.join(__dirname, "output");
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const outputFile = path.join(outputDir, `distribution-${timestamp}.json`);
    ...
    console.log(`Distribution written to ${outputFile}`);
  }
}

async function main(apiKey: string, wallet: string): Promise<void> {
