import { SwapTransactionDTO, TransferTransactionDTO, TokenDataDTO } from "../dto/TransactionDTO";
...
export class TransactionProcessorImpl {
    ...
    private identifyPlatform(data: WebhookData): SwapPlatform {
        if (data.source) {
            switch (data.source.toUpperCase()) {
                case "JUPITER": return SwapPlatform.JUPITER;
                case "RAYDIUM": return SwapPlatform.RAYDIUM;
                case "PUMP_FUN": return SwapPlatform.PUMP_FUN;
                case "ORCA": return SwapPlatform.ORCA;
                case "METEORA": return SwapPlatform.METEORA;
            }
        }
    ...
    private normalizeSwap(data: WebhookData): SwapTransactionDTO {
        ...
        const input = transfers.find(t => 
            t.fromUserAccount === this.trackedWallet && 
            !transfers.some(tt => tt.toUserAccount === this.trackedWallet && tt.mint === t.mint)
        );
        
        const output = transfers.find(t => 
            t.toUserAccount === this.trackedWallet && 
            !transfers.some(tt => tt.fromUserAccount === this.trackedWallet && tt.mint === t.mint)
        );        
    ...
    private isSwap(data: WebhookData): boolean {
        ...
        const hasInAndOut = transfers.some(t => t.fromUserAccount === this.trackedWallet) &&
    ...
    private isTransfer(data: WebhookData): boolean {
        return data.type?.toUpperCase() === "TRANSFER" || data.tokenTransfers?.length === 1;
    }

    /**
     * Processa a transação e determina se é um SWAP ou uma TRANSFERÊNCIA.
     */
    public processTransaction(webhookData: WebhookData[]): SwapTransactionDTO | TransferTransactionDTO | null {
