...
export class MeteoraImpl extends BaseNormalizerImpl {
    ...
    private readonly METEORA_PROGRAM_ID = "LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo";

    canHandle(data: WebhookData): boolean {
        return data.instructions ? data.instructions.some(instruction => 
            instruction.programId === this.METEORA_PROGRAM_ID
        ) : false;
    }
    ...
    normalize(data: WebhookData): SwapTransactionDTO {
        ...
        const input = data.tokenTransfers.find(t => 
            t.fromUserAccount === this.trackedWallet);

        const output = data.tokenTransfers.find(t => 
